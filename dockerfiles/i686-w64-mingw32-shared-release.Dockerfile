...
RUN zypper -n --gpg-auto-import-keys install \
    ...
    shadow which patch gperf wget curl git diffutils rsync \
...
RUN cd strawberry-mxe && make -j 4 MXE_TARGETS="i686-w64-mingw32.shared" MXE_BUILD_TYPE="Release" MXE_VERBOSE=1
